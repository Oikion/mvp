...
/**
 * Hook to get thread messages (replies to a parent message)
 */
export function useThreadMessages(params: {
  ...
  const key = params.enabled !== false && params.parentId
    ? `/api/messaging/messages?${queryParams.toString()}`
    : null;

  const { data, error, isLoading, isValidating, mutate } = useSWR<ThreadResponse>(
    key,
    fetcher,
    {
      revalidateOnMount: true,
...
/**
 * Get the SWR cache key for thread messages
 */
export function getThreadMessagesKey(parentId: string): string {
  return `/api/messaging/messages?parentId=${parentId}`;
}
