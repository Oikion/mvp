...
interface EventCreateFormContentProps {
  userId?: string;
  clientId?: string;
  propertyId?: string;
  onSuccess?: () => void;
...
/**
 * Event creation form content without Dialog wrapper.
 * Use this component when embedding the form inside a shared modal.
 */
export default function EventCreateFormContent({
  userId,
  clientId,
  propertyId,
  onSuccess,
  onCancel,
}: EventCreateFormContentProps) {
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : t("eventCreateForm.failedToCreateEvent");
      toast.error(message);
    }
  }
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 px-0.5">
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input
                  placeholder={t("eventCreateForm.titlePlaceholder")}
                  {...field}
                />
              </FormControl>
              <FormMessage />
            ...
            <FormItem>
              ...
              <Select onValueChange={field.onChange} value={field.value}>
                <FormControl>
                  <SelectTrigger>
                    <SelectValue
                      placeholder={t("eventCreateForm.selectUser")}
                    />
                  </SelectTrigger>
                </FormControl>
                <SelectContent>
        ...
        <div className="flex justify-end gap-2 pt-4">
          ...
          <Button type="submit" disabled={isCreating}>
            {isCreating
              ? t("eventCreateForm.creating")
              : t("eventCreateForm.createEvent")}
          </Button>
        </div>
      </form>
    </Form>
