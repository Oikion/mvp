...
export function MentionShortcutOverlay() {
  ...
  const { clients, isLoading: clientsLoading } = useClients({
    enabled: activeCategory === "clients",
  });
  const { properties, isLoading: propertiesLoading } = useProperties({
    enabled: activeCategory === "properties",
  });
  ...
  const items = React.useMemo((): EntityItem[] => {
    let rawItems: { value: string; label: string }[] = [];
  ...
  }, [activeCategory, clients, properties, documents, searchQuery]);

  // Check if loading
  const isLoading =
    (activeCategory === "clients" && clientsLoading) ||
    (activeCategory === "properties" && propertiesLoading) ||
    (activeCategory === "documents" && documentsLoading);
