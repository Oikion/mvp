...
export async function getMarketStatsByArea(organizationId?: string): Promise<Array<{
  ...
  try {
    ...
    const result = await prismadb.$queryRawUnsafe<Array<{
      area: string;
      ...
      FROM market_intel.competitor_listings
      WHERE is_active = true AND area IS NOT NULL ${orgCondition}
      GROUP BY area
      ORDER BY COUNT(*) DESC
    `);
...
export async function getPlatformStats(organizationId?: string): Promise<Array<{
  ...
  try {
    ...
    const result = await prismadb.$queryRawUnsafe<Array<{
      ...
      FROM market_intel.competitor_listings
      WHERE is_active = true ${orgCondition}
...
export async function getRecentPriceChanges(
  ...
  try {
    const typeCondition = changeType ? `AND ph.change_type = '${changeType}'` : '';
...
export async function getNewListings(
  ...
  try {
    const result = await prismadb.$queryRawUnsafe<CompetitorListing[]>(
      `SELECT * FROM market_intel.competitor_listings
       WHERE organization_id = '${organizationId}'
         AND is_active = true
