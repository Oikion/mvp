...
3. Create the server action file at `actions/{feature}/{action-name}.ts`:
...
export async function actionName(input: unknown): Promise<ActionResponse<ResultType>> {
  // Permission guard (FIRST — before any other logic)
  const guard = await requireAction("{feature}:{verb}");
  if (guard) return guard;

  // Tenant context (NEVER accept from client)
...
5. Suggest the Zod validation schema to create in `lib/validations/{feature}.ts`
6. Suggest the permission to add to `lib/permissions/action-permissions.ts`
7. Remind about i18n for any user-facing error messages
