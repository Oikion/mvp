...
export async function createEvent(
  ...
  try {
    ...
    let validatedEventType: CalendarEventType | null = null;
    if (eventType) {
      ...
      if (validTypes.includes(eventType as CalendarEventType)) {
        validatedEventType = eventType as CalendarEventType;
      }
    }
...
export async function createReminder(
  ...
  try {
    const { eventId, reminderTime } = input;
    ...
    // Calculate reminderMinutes (minutes before event start)
    const reminderMinutes = Math.round((event.startTime.getTime() - scheduledFor.getTime()) / (1000 * 60));
    if (reminderMinutes < 0) {
      return errorResponse("Reminder time must be before event start time");
    }
