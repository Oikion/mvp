...
export function DataTableRowActions<TData>({
  ...
  const onDelete = async () => {
    setLoading(true);
    try {
      await axios.delete(`/api/estate-files/${project.id}`);
    } catch (error) {
      toast({
        variant: "destructive",
  ...
  const onWatch = async () => {
    setLoading(true);
    try {
      await axios.post(`/api/estate-files/${project.id}/watch`);
    } catch (error) {
      toast({
        variant: "destructive",
  ...
  const onUnWatch = async () => {
    setLoading(true);
    try {
      await axios.post(`/api/estate-files/${project.id}/unwatch`);
    } catch (error) {
      toast({
        variant: "destructive",
        title: "Error, project not watched. Please try again.",
    ...
    <>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-[260px]">
          <DropdownMenuItem
            onClick={() => router.push(`/estate-files/boards/${project.id}`)}
          >
