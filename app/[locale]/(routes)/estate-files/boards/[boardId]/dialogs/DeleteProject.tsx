...
const DeleteProjectDialog = ({ boardId, boardName }: Props) => {
  ...
  const onDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`/api/estate-files/${boardId}`);
      toast({
        title: "Success",
        description: `Estate file: ${boardName} deleted successfully`,
      });
    } catch (error) {
      toast({
        variant: "destructive",
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="">
        <DialogHeader>
          <DialogTitle className="p-2">Delete Estate File</DialogTitle>
          <DialogDescription className="p-2">
            Are you sure you want to delete this estate file? You will not be able
