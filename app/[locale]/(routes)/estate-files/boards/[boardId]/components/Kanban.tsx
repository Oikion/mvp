...
const Kanban = (props: any) => {
  ...
  const onDragEnd = async ({ source, destination }: DropResult) => {
    ...
    try {
      setData(data);
      await axios.put(`/api/estate-files/tasks/update-kanban-position`, {
  ...
  const onDeleteSection = async () => {
    ...
    try {
      await axios.delete(`/api/estate-files/sections/delete-section/${sectionId}`);
  ...
  const updateSectionTitle = async (
    ...
    timer = setTimeout(async () => {
      try {
        ...
        await axios.put(`/api/estate-files/sections/update-title/${sectionId}`, {
          newTitle,
        });
        toast({
  ...
  const createTask = async (sectionId: string) => {
    ...
    try {
      await axios.post(
        `/api/estate-files/tasks/create-task/${boardId}`,
        {
          section: sectionId,
        }
      );
  ...
  const onDelete = async () => {
    ...
    try {
      await axios.delete(`/api/estate-files/tasks/`, {
        data: {
          id: selectedTask.id,
