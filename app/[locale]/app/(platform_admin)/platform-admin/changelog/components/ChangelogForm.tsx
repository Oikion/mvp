...
export function ChangelogForm({ entry, categories: initialCategories, onSuccess, onCancel }: ChangelogFormProps) {
  ...
  // Template state
  const [selectedTemplate, setSelectedTemplate] = useState<ChangelogTemplate | null>(null);
  ...
  // Fetch latest version on mount (only for new entries)
  useEffect(() => {
    if (!entry) {
  ...
  // Update suggested version when template or latest version changes
  useEffect(() => {
    if (selectedTemplate && !entry) {
      const suggested = suggestNextVersion(latestVersion, selectedTemplate.versionBump);
      setSuggestedVersion(suggested);
    } else {
      setSuggestedVersion(null);
    }
  }, [selectedTemplate, latestVersion, entry]);
  ...
  // Find category by name (for template recommendations)
  const findCategoryByName = (name: string): ChangelogCategoryData | undefined => {
  ...
  const applyTemplate = (template: ChangelogTemplate) => {
    ...
    const recommendedCategory = findCategoryByName(template.recommendedCategoryName);
    if (recommendedCategory) {
      form.setValue("customCategoryId", recommendedCategory.id);
    }

    // Add default tags (avoid duplicates)
    const existingTagNames = new Set(tags.map((t) => t.name.toLowerCase()));
    const newTags = template.defaultTags.filter(
    ...
    if (suggestedVersion || latestVersion) {
      const version = suggestNextVersion(latestVersion, template.versionBump);
  ...
  // Apply suggested version to form
  const applySuggestedVersion = () => {
    if (suggestedVersion) {
      form.setValue("version", suggestedVersion);
      toast.success(`Version set to ${suggestedVersion}`);
    }
  };
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        {/* Template Selector - Only show for new entries */}
        {!entry && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
              {CHANGELOG_TEMPLATES.map((template) => {
                const style = getTagStyle(template.color);
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "flex flex-col items-start p-3 rounded-lg border text-left transition-all hover:shadow-sm",
                      isSelected
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 mt-2 text-xs">
                      <Badge variant="outline" className="text-[10px] px-1.5 py-0">
                        {template.versionBump.toUpperCase()}
                      </Badge>
                      <span className="text-muted-foreground">
                        → {template.recommendedCategoryName}
                      </span>
                    </div>
                  </button>
                );
            ...
            {selectedTemplate && (
              <p className="text-xs text-muted-foreground">
                Template applied. All fields have been pre-filled but you can customize them.
              </p>
            )}
            <Separator />
          </div>
                ...
                {!entry && (
                  <div className="space-y-1">
                    {isLoadingVersion ? (
                      ...
                      <p className="text-xs text-muted-foreground">
                        Latest published: <span className="font-mono font-medium">v{latestVersion}</span>
                      </p>
                    ) : (
                      <p className="text-xs text-muted-foreground">
