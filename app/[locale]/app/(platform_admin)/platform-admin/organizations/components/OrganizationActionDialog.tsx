...
export function OrganizationActionDialog({
  ...
  const handleSubmit = async () => {
    if (!reason.trim()) {
      toast.error("Reason required", { description: "Please provide a reason for this action.", isTranslationKey: false });
      return;
    }

    if (config.requiresConfirm && confirmText !== "DELETE") {
      toast.error("Confirmation required", { description: "Please type DELETE to confirm.", isTranslationKey: false });
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        toast.info("Action completed", { description: config.successMessage, isTranslationKey: false });
        onOpenChange(false);
        router.refresh();
      } else {
        toast.error("Action failed", { description: result.error, isTranslationKey: false });
      }
    } catch (error) {
      toast.error("Error", { description: "An unexpected error occurred.", isTranslationKey: false });
    } finally {
      setIsLoading(false);
    }
  };
