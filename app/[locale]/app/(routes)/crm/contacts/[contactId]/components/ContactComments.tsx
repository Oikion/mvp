...
} from "@/hooks/swr/useContactComments";
...
export function ContactComments({
  ...
  const { comments, isLoading, mutate } = useContactComments(contactId);
  const { addComment, isAdding } = useAddContactComment(contactId);
  const { deleteComment, isDeleting } = useDeleteContactComment(contactId);
