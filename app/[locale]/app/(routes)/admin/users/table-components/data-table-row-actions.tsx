...
export function DataTableRowActions<TData>({
  ...
  const onCopy = (id: string) => {
    ...
    toast.info(t("idCopied") || "ID copied", { isTranslationKey: false });
  };
  ...
  const onRemoveMember = async () => {
    ...
    try {
      ...
      if (memberToRemove) {
        ...
        router.refresh();
        toast.success(t("memberRemoved") || "Member removed", { isTranslationKey: false });
      } else {
        toast.error(t("memberNotFound") || "Member not found", { isTranslationKey: false });
      }
    } catch (error: unknown) {
      console.error("Remove member error:", error);
      const clerkError = error as { errors?: Array<{ message?: string }> };
      toast.error(t("removeMemberError") || "Failed to remove member", { description: clerkError?.errors?.[0]?.message, isTranslationKey: false });
    } finally {
      setLoading(false);
      setRemoveOpen(false);
    }
  ...
  const onPromoteToAdmin = async () => {
    ...
    try {
      ...
      if (memberToUpdate) {
        ...
        router.refresh();
        toast.success(t("promotedToAdmin") || "Promoted to admin", { isTranslationKey: false });
      }
    } catch (error: unknown) {
      console.error("Promote to admin error:", error);
      const clerkError = error as { errors?: Array<{ message?: string }> };
      toast.error(t("promoteError") || "Failed to promote", { description: clerkError?.errors?.[0]?.message, isTranslationKey: false });
    } finally {
      setLoading(false);
    }
  };
  ...
  const onDemoteFromAdmin = async () => {
    ...
    try {
      ...
      if (memberToUpdate) {
        ...
        router.refresh();
        toast.success(t("demotedFromAdmin") || "Demoted from admin", { isTranslationKey: false });
      }
    } catch (error: unknown) {
      console.error("Demote from admin error:", error);
      const clerkError = error as { errors?: Array<{ message?: string }> };
      toast.error(t("demoteError") || "Failed to demote", { description: clerkError?.errors?.[0]?.message, isTranslationKey: false });
    } finally {
      setLoading(false);
    }
  };
