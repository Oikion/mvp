...
export function EditableField({
  ...
  // Callback ref for input fields
  const inputCallbackRef = (node: HTMLInputElement | null) => {
    inputRef.current = node;
    if (fieldKey && registerFieldRef && placeholder.type !== "select") {
      registerFieldRef(fieldKey, node);
    }
  };
  
  // Callback ref for select trigger
  const selectTriggerCallbackRef = (node: HTMLButtonElement | null) => {
    selectTriggerRef.current = node;
    if (fieldKey && registerFieldRef && placeholder.type === "select") {
      registerFieldRef(fieldKey, node);
    }
  };
  ...
  useEffect(() => {
    return () => {
      if (fieldKey && registerFieldRef) {
        registerFieldRef(fieldKey, null);
      }
    };
  }, [fieldKey, registerFieldRef]);
