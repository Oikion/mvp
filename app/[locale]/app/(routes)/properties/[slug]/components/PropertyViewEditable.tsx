...
import { PropertyComments } from "../../../mls/properties/[slug]/components/PropertyComments";
import {
  usePropertyLinked,
...
export function PropertyViewEditable({ 
  ...
  const [formData, setFormData] = useState({
    ...
    size_net_sqm: data.size_net_sqm?.toString() || "",
  ...
  const EditableNumber = ({ 
    ...
    if (value) {
      displayValue = currency
        ? format.number(Number.parseFloat(value), { style: "currency", currency })
        : `${prefix}${format.number(Number.parseFloat(value))}${suffix}`;
    }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="-mx-2 flex items-start justify-between space-x-4 rounded-md p-2 transition-all hover:bg-accent hover:text-accent-foreground">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {data.updatedAt
                        ? format.dateTime(new Date(data.updatedAt), {
                            dateStyle: "medium",
                            timeStyle: "medium",
                          })
                        : "N/A"}
