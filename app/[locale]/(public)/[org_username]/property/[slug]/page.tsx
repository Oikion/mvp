...
interface OrgPropertyPageProps {
  params: Promise<{ org_username: string; slug: string; locale: string }>;
}
...
export async function generateStaticParams() {
  ...
  try {
    ...
    const params: { org_username: string; slug: string; locale: string }[] = [];
    ...
    for (const org of organizations) {
      ...
      for (const property of properties) {
        if (property.id) {
          params.push({ org_username: org.slug, slug: property.friendlyId, locale: "en" });
          params.push({ org_username: org.slug, slug: property.friendlyId, locale: "el" });
        }
      }
    }
...
export async function generateMetadata({
  ...
  const { org_username, slug, locale } = await params;
  const property = await getPublicPropertyByOrgSlug(org_username, slug);
  ...
  const canonicalUrl = `${baseUrl}/${locale}/${org_username}/property/${slug}`;

  const description = [
    property.property_type,
    property.bedrooms ? `${property.bedrooms} υπνοδωμάτια` : null,
    property.size_net_sqm || property.size_net_sqm
      ? `${property.size_net_sqm || property.size_net_sqm} τ.μ.`
      : null,
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        en: `${baseUrl}/en/${org_username}/property/${slug}`,
        el: `${baseUrl}/el/${org_username}/property/${slug}`,
      },
    },
    openGraph: {
      type: "website",
