...
import { errorMessage } from "@/lib/safe-error";
...
export async function POST(request: NextRequest) {
  try {
    const body = await request.json() as { userMessage?: unknown; conversationState?: ConversationState };
    ...
    logger.error("[VOICE_CONVERSATION] Conversation error", { error });
    return NextResponse.json(
      { error: errorMessage(error) || "Failed to process conversation" },
      { status: 500 }
    );
  }
}
