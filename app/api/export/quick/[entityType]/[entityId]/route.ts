...
function generateXeXml(property: PropertyData, locale: string): string {
  ...
  xml += `<properties>\n`;
  xml += `  <property>\n`;
  xml += `    <id>${escapeXml(property.friendlyId)}</id>\n`;
...
export async function GET(
  ...
  try {
    ...
    // Validate inputs
    if (!["property", "contact"].includes(entityType)) {
    ...
    if (entityType === "contact") {
      ...
      // For contacts, only PDF flyer is supported for now
      if (format !== "pdf-flyer") {
        return NextResponse.json(
          { error: "Only PDF flyer export is supported for contacts" },
          { status: 400 }
        );
      }
      ...
      const labels = {
        ...
        details: locale === "el" ? "Λεπτομέρειες" : "Details",
        email: "Email",
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              View,
              { style: styles.header },
              React.createElement(Text, { style: styles.title }, contact.displayName),
              React.createElement(Text, { style: styles.subtitle }, contact.category?.[0] ?? "Contact")
            ),
            React.createElement(
              View,
              { style: styles.section },
              React.createElement(Text, { style: styles.sectionTitle }, labels.contactInfo),
              contact.email && React.createElement(View, { style: styles.row },
                React.createElement(Text, { style: styles.label }, labels.email),
                React.createElement(Text, { style: styles.value }, contact.email)
              ),
              contact.primaryPhone && React.createElement(View, { style: styles.row },
                React.createElement(Text, { style: styles.label }, labels.phone),
                React.createElement(Text, { style: styles.value }, contact.primaryPhone)
              ),
              billing?.street && React.createElement(View, { style: styles.row },
                React.createElement(Text, { style: styles.label }, labels.address),
                React.createElement(Text, { style: styles.value },
                  [billing.street, billing.city, billing.postalCode].filter(Boolean).join(", ")
                )
              )
            ),
            React.createElement(
              ...
              React.createElement(View, { style: styles.row },
                ...
                React.createElement(Text, { style: styles.value }, contact.status ?? "LEAD")
              ),
              contact.assignedAgent?.name && React.createElement(View, { style: styles.row },
                React.createElement(Text, { style: styles.label }, labels.assignedTo),
                React.createElement(Text, { style: styles.value }, contact.assignedAgent.name)
              )
            ),
            React.createElement(
              View,
      ...
      const pdfDoc = pdf(ContactDocument());
      const blob = await pdfDoc.toBlob();
      const arrayBuffer = await blob.arrayBuffer();
      const content = Buffer.from(arrayBuffer);
      const filename = `contact_${contact.displayName.replace(/[^a-zA-Z0-9]/g, "_")}_card.pdf`;
