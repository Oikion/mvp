...
export async function GET(req: Request) {
  try {
    ...
    // Handle connection errors (common in serverless)
  ...
  try {
    ...
    if (propertyIds && Array.isArray(propertyIds) && propertyIds.length > 0) {
      ...
      if (validProperties.length > 0) {
        relations.Properties = {
          connect: validProperties.map((property) => ({ id: property.id })),
        };
      }
    }
    ...
    if (documentIds && Array.isArray(documentIds) && documentIds.length > 0) {
      ...
      if (validDocuments.length > 0) {
        relations.Documents = {
          connect: validDocuments.map((doc) => ({ id: doc.id })),
        };
      }
    }
    ...
    if (error?.code === 'P2002') {
      return NextResponse.json(
        { error: 'A calendar event with this ID already exists' },
        { status: 409 }
      );
    }
    if (error?.code === 'P2003') {
      return NextResponse.json(
        { error: 'Invalid reference to a related record (client, property, or task)' },
        { status: 400 }
      );
    }
    if (error?.code === 'P2025') {
